...
def getLink(soup):
    ...
    olympics = soup.find('a', title='List of American universities with Olympic medalist students and alumni')
...
def getAdmissionsInfo2019(soup):
    ...
    college_table = soup.find('table', class_='toccolours')
    colleges = college_table.find_all('tr')
    d = {}

    for x in colleges:
        college_row = x.find_all('td')
        key = college_row[0].text
        value = college_row[1].text
        d[key] = value
